...
- **数据简介**：
    * 包含在线和离线两类手写数据，`HWDB1.0~1.2`总共有3,895,135个手写单字样本，分属7,356类（7,185个汉字和171个英文字母、数字、符号）；`HWDB2.0~2.2`总共有5,091页图像，分割为52,230个文本行和1,349,414个文字。所有文字和文本样本均存为灰度图像。部分单字样本图片如下所示。
...
- **数据简介**: NIST19数据集适用于手写文档和字符识别的模型训练，从3,600位作者的手写样本表格中提取得到，总共包含81w张字符图片。其中9张图片示例如下。
