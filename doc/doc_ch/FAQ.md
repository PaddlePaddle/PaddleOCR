...
**A**：可以参考[配置文件](../../configs/rec/ch_ppocr_v2.0/rec_chinese_lite_train_v2.0.yml)在Train['dataset']['transforms']添加`BaseDataAugmentation`字段，使数据增强生效。可以通过设置x_prob(x表示数据增强的名称，概率默认0.4)等参数来设置每种数据增强采用的概率。详细设置可以参考[ISSUE 1744](https://github.com/PaddlePaddle/PaddleOCR/issues/1744)。
