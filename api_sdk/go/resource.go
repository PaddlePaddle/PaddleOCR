...
// WithOverwrite allows SaveResource to replace an existing destination file.
...
// SaveResource downloads a resource URL to dest and returns the saved file path.
//
// If dest is an existing directory, the filename is derived from the resource
// URL path. Existing files are not overwritten unless WithOverwrite(true) is
// provided. Resource downloads intentionally omit PaddleOCR official API authorization
// headers so presigned or object-storage URLs are fetched without API tokens.
func (c *Client) SaveResource(ctx context.Context, resourceURL, dest string, opts ...SaveResourceOption) (string, error) {
	...
	defer cancel()
	req, err := http.NewRequestWithContext(reqCtx, http.MethodGet, resourceURL, nil)
	if err != nil {
	...
	if _, err := io.Copy(tmp, resp.Body); err != nil {
		_ = tmp.Close()
		return "", mapBodyReadError("download resource", err)
	}
...
// SaveOCRResultResources downloads all OCR image resources in result to destDir.
//
// destDir must be an existing directory. Empty OCR image URLs are skipped.
// Filenames are stable, using ocr-page-{n} plus a safe extension derived from
// the resource URL when available.
...
// SaveDocumentParsingResultResources downloads markdown and output image resources in result.
//
// destDir must be an existing directory. Map keys are used as filenames after
// strict validation.
...
func validateResultResourceFilename(name string) error {
	if name == "" || name == "." {
		return &InvalidRequestError{PaddleOCRAPIError{Message: "resource filename is required"}}
	}
	if filepath.IsAbs(name) || path.IsAbs(name) || strings.ContainsAny(name, `/\`) {
