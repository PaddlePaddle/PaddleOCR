...
def GetBcgAna(api_key, img_path, img_base, save_path, prompt):
    ...
    try:
        ...
        save_rsp_to_json(response_content, os.path.join(save_path, 'rsp.txt'))
...
def main():
    ...
    # main_dir="/home/ubuntu/moe/PaddleOCR_m/constructor/result/pdf_test/batch"
    # main_dir="/home/ubuntu/moe/PaddleOCR_m/constructor/result/pdf_test/new_week_in_charts"
    main_dir="/home/ubuntu/moe/PaddleOCR_m/constructor/result/pdf_test/new_week_in_charts/"
    new_key = '50ac1256cb84c79ec648ab975530dbef.zH4dIC7KOQK8yLgU'
    prompt0 = "这是图片,请尽可能多的描述有关该图像的内容。"
  
    doc_dirs=sorted([d for d in os.listdir(main_dir) 
                     if os.path.isdir(os.path.join(main_dir, d)) and d !="img"])
    ...
    for doc_dir in doc_dirs:
        doc_path = os.path.join(main_dir, doc_dir)
        print("====================================")
        print(f"开始处理文档文件夹 {doc_dir}")

        # 获取文档文件夹下的所有页面文件夹
        page_dirs= sorted([d for d in os.listdir(doc_path) 
                           if os.path.isdir(os.path.join(doc_path, d)) and d !="img"])

        if not page_dirs:
            print(f"没有找到任何页面文件夹在 {doc_dir}")
            continue

        # 遍历下面的每个页面文件夹
        for page_dir in page_dirs:
            process_images(page_dir, doc_path, new_key, prompt0)
    print("所有文档和页面分析均已结束")
