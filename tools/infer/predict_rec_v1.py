...
def main(args, bidi_state):
    ...
    try:
        ...
        exit()
    # 修改源码: 将结果写入txt
    with open(os.path.join(os.path.dirname(args.image_dir), "predict.txt"), 'w', encoding='utf8') as f:
        for ino in range(len(img_list)):
            logger.info("Predicts of {}:{}".format(valid_image_file_list[ino],
                                                   rec_res[ino]))
            label = rec_res[ino][0]
            if bidi_state is True:
                label = get_display(label)
            f.write("{0}\t{1}\n".format(valid_image_file_list[ino].replace("\\", "/").replace("./dataset/", "").replace("C:/Users/lvjia/Pictures/dataset/test/line/", ""), label))
    if args.benchmark:
        text_recognizer.autolog.report()


if __name__ == "__main__":
    main(utility.parse_args(), False)
