...
############## Test Refacor
def eval(model, valid_dataloader, post_process_class, eval_class, model_type=None, extra_input=False, scaler=None, amp_level='O2', amp_custom_black_list=[], amp_custom_white_list=[], amp_dtype='float16'):
    model.eval()
    with paddle.no_grad():
        total_frame = 0.0
        total_time = 0.0
        ...
        for idx, batch in enumerate(valid_dataloader):
            ...
            if scaler:
                with paddle.amp.auto_cast(level=amp_level, custom_black_list=amp_custom_black_list, dtype=amp_dtype):
                    preds = process_batch(model, model_type, images, batch, extra_input)
            else:
                preds = process_batch(model, model_type, images, batch, extra_input)

            total_time += time.time() - start
            eval_results(post_process_class, eval_class, model_type, preds, batch)
...
def process_batch(model, model_type, images, batch, extra_input):
    if model_type == 'table' or extra_input:
        ...
        preds = model(batch)
        return preds["sr_img"], preds["lr_img"]
    else:
        return model(images)

def eval_results(post_process_class, eval_class, model_type, preds, batch):
    batch_numpy = [item.numpy() if isinstance(item, paddle.Tensor) else item for item in batch]

    if model_type in ['table', 'kie']:
        result = post_process_class(preds, batch_numpy) if post_process_class else preds
        eval_class(result, batch_numpy)
    elif model_type in ['sr']:
        eval_class(preds, batch_numpy)
    elif model_type in ['can']:
        eval_class(preds[0], batch_numpy[2:], epoch_reset=(idx == 0))
    else:
        result = post_process_class(preds, batch_numpy[1])
        eval_class(result, batch_numpy)
