...
使用下述命令将数据集划分为训练集和测试集, 这里将90.00%划分为训练集，10.00%划分为测试集
...
|类型|数量|图片地址|标注文件路径|
|---|---|---|---|
|训练集|18,000|/home/aistudio/data/data165849/table_gen_dataset|/home/aistudio/train.txt|
|测试集|2,000|/home/aistudio/data/data165849/table_gen_dataset|/home/aistudio/val.txt|
...
SLANet是PP-StructureV2全新推出的表格识别模型，相比PP-StructureV1中TableRec-RARE，在速度不变的情况下精度提升4.70%。TEDS提升2.00%
...
### 3.1 代码、环境、数据准备
#### 3.1.1 代码准备
首先，我们需要准备训练表格属性的代码，PaddleClas集成了PULC方案，该方案可以快速获得一个在CPU上用时2.00ms的属性识别模型。PaddleClas代码可以clone下载得到。获取方式如下：
