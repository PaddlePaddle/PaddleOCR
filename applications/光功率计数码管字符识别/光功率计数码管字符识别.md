...
针对以上问题， 本例选用 PP-OCRv3 和 SVTR_Tiny 两个高精度模型训练，同时提供了真实数据挖掘案例和数据合成案例。基于 PP-OCRv3 模型，在构建的真实评估集上精度从 52.00% 提升至 72.00%，SVTR_Tiny 模型精度可达到 78.90%。
...
`ppocr/utils/ppocr_keys_v1.txt` 是一个包含6,623个字符的中文字典
