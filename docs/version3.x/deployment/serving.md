...
服务默认以 Base64 编码内联返回响应中的图像与文件字段（如 `outputImages`、`inputImage`、`markdown.images`、`exports` 等）。当响应包含较大文件或多页 PDF 时，Base64 会显著增加响应体积。可在产线配置文件的 `Serving.extra` 节中配置对象存储，将相应字段改为以预签名 URL 返回：
