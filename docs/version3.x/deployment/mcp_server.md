...
本节将以 **Claude for Desktop** 作为 MCP 主机（host），并以 **星河社区服务** 工作模式为例，引导您完成快速配置。此模式无需在本地安装复杂的依赖，推荐新用户使用。请参考 [3. 配置说明](#3-配置说明) 了解其他工作模式的操作流程以及更多配置项。
...
在 host 的配置文件中（如 `claude_desktop_config.json`），您需要定义 MCP 服务器的启动方式。关键字段如下：
