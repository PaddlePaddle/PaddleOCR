...
以 opencv 4.7.0为例，[opencv-4.7.0-windows.exe下载地址](https://github.com/opencv/opencv/releases/download/4.7.0/opencv-4.7.0-windows.exe)下载解压后，会在当前的文件夹中生成 `opencv/` 的子文件夹，其中 `opencv/build` 为预编译库，在后续编译通用 OCR 产线 demo 时，将作为 OpenCV 安装库的路径使用。
