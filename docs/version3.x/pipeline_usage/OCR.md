...
在 200 张图像（包括通用图像与文档图像）上测试。对于每项指标，测量 3 次取平均值。测试时从磁盘读取图像，因此读图时间及其他额外开销也被包含在总耗时内。如果将图像提前载入到内存，可进一步减少平均每图约 25 ms 的时间开销。
...
### 5.1 PP-OCRv5 与 PP-OCRv4 推理性能对比
...
| 产线配置 | 平均每图耗时（s） | 平均每秒预测字符数量 | 平均 CPU 利用率（%） | 峰值 RAM 用量（MB） | 平均 RAM 用量（MB） | 峰值 VRAM 用量（MB） | 平均 VRAM 用量（MB） |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| 产线配置 | 平均每图耗时（s） | 平均每秒预测字符数量 | 平均 CPU 利用率（%） | 峰值 RAM 用量（MB） | 平均 RAM 用量（MB） |
| --- | --- | --- | --- | --- | --- |
...
> 说明：文本图像矫正等辅助功能会对产线推理精度造成影响，因此并不一定使用的辅助功能越多、资源用量越大。

### 5.3 文本检测模块输入缩放尺寸策略对 PP-OCRv5 推理性能的影响
