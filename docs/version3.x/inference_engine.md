...
PaddleOCR 3.5 引入了统一的推理引擎配置方式：使用 `engine` 选择底层推理后端，使用 `engine_config` 传递该后端的专属配置。无论是单模型还是产线，均可按这套方式声明推理行为。

如果不显式指定 `engine`，默认行为与旧版本保持一致：除高性能推理功能和生成式 AI 客户端请求功能等少数场景外，大多数情况下会优先使用飞桨框架推理；如果显式指定 `engine`，则会优先按指定引擎初始化。

## 1. 什么是推理引擎

在 PaddleOCR 中，推理引擎指模型执行时所使用的底层运行时。它决定了模型由哪套运行时加载与执行，可以将它理解为“模型推理时实际使用的后端”。在使用推理引擎时，用户通常只需要关心两件事：
...
`engine` 用于指定推理后端，可取值如下：

| 取值 | 含义 | 说明 |
| - | - | - |
| `None` | 不显式指定引擎 | 默认行为与旧版本 PaddleOCR 保持一致：除高性能推理功能和生成式 AI 客户端请求功能等少数场景外，大多数情况下会优先使用飞桨框架推理。 |
