...
**PaddleOCR-VL-1.5** 是一个升级版模型，在 OmniDocBench v1.5 上取得了 94.5% 的新 SOTA（最佳）准确率。为了严格评估其对现实世界物理畸变的鲁棒性——包括扫描伪影、倾斜、弯曲、屏摄和光照变化——我们提出了 Real5-OmniDocBench 基准测试。实验结果表明，该增强模型在这一新构建的基准测试中达到了 SOTA 性能。此外，我们通过加入印章识别和文字检测识别任务扩展了模型能力，同时保持了 0.9B 的超紧凑 VLM 规模和高效率。
...
4. **强化多元素识别能力：** PaddleOCR-VL-1.5 进一步增强了在特定场景和多语言识别方面的能力。针对生僻字、古籍、多语言表格、下划线和复选框的识别性能得到提升，语言覆盖范围扩展至包括中国藏文和孟加拉语。
