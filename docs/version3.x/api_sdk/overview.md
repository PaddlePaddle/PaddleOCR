...
目前提供 Python、TypeScript、Go SDK，以及集成在 PaddleOCR CLI 中的 `paddleocr api` 命令。源码相邻参考见 [Python SDK 参考](../../../api_sdk/PYTHON.md)、[TypeScript SDK 参考](../../../api_sdk/typescript/README.md) 与 [Go SDK 参考](../../../api_sdk/go/README.md)。各语言用户文档入口如下：
...
- Python：适合已经使用 `paddleocr` Python 包的项目，提供同步 `APIClient` 与异步 `AsyncAPIClient`。
...
文档解析任务的 `model` 参数可选，各 SDK 与 CLI 默认使用 `PaddleOCR-VL-1.5`。当前支持的模型包括 `PP-StructureV3`、`PaddleOCR-VL` 和 `PaddleOCR-VL-1.5`。

## 结果与资源

便捷调用会提交任务、等待完成、下载结果 JSONL 并解析为类型化结果对象。显式提交接口适合需要非阻塞状态查询、并发等待或自定义调度的场景。

结果中的图片、Markdown 资源或其他输出资源可以通过 `save_resource` / `saveResource` / `SaveResource` 等帮助函数保存到本地。资源下载不会向结果资源 URL 发送 PaddleOCR 官方 API 鉴权头，默认也不会覆盖已有文件。
