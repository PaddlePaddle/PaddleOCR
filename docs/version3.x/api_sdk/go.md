...
- `SubmitOCR(...)`：只提交 OCR 任务，返回 `Operation`。
- `SubmitDocumentParsing(...)`：只提交文档解析任务，返回 `Operation`。
...
`WithRequestTimeout` 限制一次 HTTP 请求，包括提交、查询状态和下载资源。`WithPollTimeout` 限制 `OCR`、`ParseDocument`、`WaitOCRResult`、`WaitDocumentParsingResult` 以及 `Operation` 等待方法的总等待时间。调用方也可以通过 `context.Context` 取消请求。

## 模型扩展

OCR 的 `Model` 可省略，默认是 `PPOCRv5`。当前 PaddleOCR 官方 API 版本只开放 PP-OCRv5 作为 OCR 模型。

文档解析的 `Model` 可选，默认使用 `PaddleOCRVL15`。支持的文档解析模型包括 `PPStructureV3`、`PaddleOCRVL` 和 `PaddleOCRVL15`。SDK 通过 `IsOCRModel` 与 `IsDocumentParsingModel` 集中校验模型类型，未来模型可在模型分类处集中添加。
...
更多源码相邻参考见 [api_sdk/go/README.md](../../../api_sdk/go/README.md)。
