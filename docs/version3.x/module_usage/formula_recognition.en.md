...
<th>Avg-BLEU(%)</th>
<th>GPU Inference Latency (ms)<br/>[Standard Mode / High-Performance Mode]</th>
<th>CPU Inference Latency (ms)<br/>[Standard Mode / High-Performance Mode]</th>
<th>Model Size (MB)</th>
<th>Description</th>
</tr>
<tr>
<td>UniMERNet</td>
<td>
<a href="https://paddle-model-ecology.bj.bcebos.com/paddlex/official_inference_model/paddle3.0.0/UniMERNet_infer.tar">Inference Model</a> /
<a href="https://paddle-model-ecology.bj.bcebos.com/paddlex/official_pretrained_model/UniMERNet_pretrained.pdparams">Training Model</a>
</td>
<td>85.91</td>
<td>2266.96 / -</td>
<td>- / -</td>
<td>1.4 G</td>
<td>UniMERNet is a formula recognition model developed by Shanghai AI Lab.  
It uses Donut Swin as the encoder and MBartDecoder as the decoder, and is trained on a dataset of one million formulas including simple formulas, complex formulas, scanned formulas, and handwritten formulas.  
This greatly improves the model's recognition accuracy for formulas in real-world scenarios.</tr>
<tr>
<td>PP-FormulaNet-S</td><td><a href="https://paddle-model-ecology.bj.bcebos.com/paddlex/official_inference_model/paddle3.0.0/PP-FormulaNet-S_infer.tar">Inference Model</a> /<a href="https://paddle-model-ecology.bj.bcebos.com/paddlex/official_pretrained_model/PP-FormulaNet-S_pretrained.pdparams">Training Model</a>
</td>
<td>87.00</td>
<td>202.25 / -</td>
<td>- / -</td>
<td>223.6 M</td>
<td rowspan="2">PP-FormulaNet is an advanced formula recognition model developed by the PaddlePaddle Vision Team at Baidu.  
It supports recognition of 50,000 common LaTeX source tokens.  
The PP-FormulaNet-S version uses PP-HGNetV2-B4 as its backbone network and adopts techniques such as parallel masking and model distillation, significantly improving inference speed while maintaining high recognition accuracy.  
It is suitable for scenarios such as simple printed formulas and multi-line simple printed formulas.  The PP-FormulaNet-L version is based on Vary_VIT_B as the backbone and is extensively trained on large-scale formula datasets.  
Compared with PP-FormulaNet-S, it shows significant improvement in recognizing complex formulas, and is suitable for simple printed formulas, complex printed formulas, and handwritten formulas.</td>
</tr>
<tr><td>PP-FormulaNet-L</td><td><a href="https://paddle-model-ecology.bj.bcebos.com/paddlex/official_inference_model/paddle3.0.0/PP-FormulaNet-L_infer.tar">Inference Model</a> /<a href="https://paddle-model-ecology.bj.bcebos.com/paddlex/official_pretrained_model/PP-FormulaNet-L_pretrained.pdparams">Training Model</a></td>
<td>90.36</td>
<td>1976.52 / -</td>
<td>- / -</td>
<td>694.6 M</td>
...
<td>- / -</td>
<td>697.6 M</td>
<td rowspan="3">PP-FormulaNet_plus is an enhanced version developed based on PP-FormulaNet.  This version uses a rich dataset collected from Chinese theses, professional books, exam papers, and mathematical journals, significantly improving recognition capability.  Among them, PP-FormulaNet_plus-M and PP-FormulaNet_plus-L add support for Chinese formulas and increase the maximum number of prediction tokens from 1024 to 2560, greatly improving performance in recognizing complex formulas.  PP-FormulaNet_plus-S focuses on enhancing English formula recognition capabilities.  The PP-FormulaNet_plus series models perform better in handling complex and diverse formula recognition tasks.
</td>
</tr>
<tr>
...
<td>- / -</td>
<td>89.7 M</td>
<td>LaTeX-OCR is a formula recognition algorithm based on autoregressive large models.  By using Hybrid ViT as the backbone network and transformer as the decoder, it significantly improves the accuracy of formula recognition.</td>
</tr>
</table>
...
* `FormulaRecognition` instantiates the formula recognition model (here using `PP-FormulaNet-S` as an example), with detailed description as follows:
<table>
<thead>
<tr>
<th>Parameter</th>
...
### 4.3 Model Training
PaddleOCR is modularized. To train the `PP-FormulaNet-S`  model, you need to use its [config file](https://github.com/PaddlePaddle/PaddleOCR/blob/main/configs/rec/PP-FormuaNet/PP-FormulaNet-S.yaml).
...
You can evaluate trained weights, e.g., output/xxx/xxx.pdprams, or use the downloaded [model](https://paddleocr.bj.bcebos.com/contribution/rec_ppformulanet_s_train.tar ) with the following command:
...
After exporting, the static graph model will be saved in `./PP-FormulaNet-S_infer/`, and you will see the following files:
 ```
