...
2. **Guide the user to obtain credentials**: Visit the [PaddleOCR website](https://www.paddleocr.com), click **API**, select a model (`PP-StructureV3`, `PaddleOCR-VL`, `PaddleOCR-VL-1.5`, or `PaddleOCR-VL-1.6`), then copy the `API_URL` and `Token`. They map to these environment variables:
