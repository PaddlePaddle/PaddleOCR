...
class RandomPerspective(object):
    ...
    def __init__(self, prob=0.3, degrees=0.0, scale=0.2, shear=5.0,
                 perspective=0.0002, fit_output=True,
                 fill_value=(123.675, 116.28, 103.53), min_area_ratio=0.1,
                 **kwargs):
        self.prob = prob
    ...
    def __call__(self, data):
        ...
        transformed_im = cv2.warpPerspective(
            im, M, target_size, flags=cv2.INTER_LINEAR,
        ...
        if len(polys) > 0:
            ...
            orig_areas = np.array([cv2.contourArea(p.astype(np.float32))
                                   for p in np.array(polys, dtype=np.float32)])
            new_areas = np.array([cv2.contourArea(p.astype(np.float32))
                                  for p in warped_pts])
            ...
            valid = ((new_areas > orig_areas * self.min_area_ratio) &
                     (centers_x > 0) & (centers_x < tw) &
                     (centers_y > 0) & (centers_y < th))
    ...
    def _get_core_matrix(self, h, w):
        ...
        # 将 perspective 系数归一化：以 640px 为参考尺寸，
        # 确保不同大小图片的透视变形比例一致
        ref_size = 640.0
        max_dim = max(h, w)
