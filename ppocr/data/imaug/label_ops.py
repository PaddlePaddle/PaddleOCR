...
class VQATokenLabelSeqEncode(object):
    """
    Label encode for NLP VQA methods
    """
    ...
    def __init__(self,
                 ...
                 ocr_engine=None,
                 **kwargs):
        super(VQATokenLabelSeqEncode, self).__init__()
        from paddlenlp.transformers import LayoutXLMTokenizer, LayoutLMTokenizer, LayoutLMv2Tokenizer
        from ppocr.utils.utility import load_vqa_seq_label_maps
        tokenizer_dict = {
            'LayoutXLM': {
                'class': LayoutXLMTokenizer,
                'pretrained_model': 'layoutxlm-base-uncased'
        ...
        self.label2id_map, id2label_map = load_vqa_seq_label_maps(class_path)
        self.add_special_ids = add_special_ids
        self.infer_mode = infer_mode
        self.ocr_engine = ocr_engine
        self.ignore_idx = paddle.nn.CrossEntropyLoss().ignore_index
    ...
    def __call__(self, data):
        ...
        for info in ocr_info:
            ...
            if not self.infer_mode:
                label = info['label']
                gt_label = self._parse_label_new(label, encode_res)

            if train_re:
                if gt_label[0] != self.label2id_map["other"]:
                    entity_id_to_index_map[info["id"]] = len(entities)
                    label = label.upper()
                    entities.append({
                        "start": len(input_ids_list),
    ...
    def _parse_label_new(self, label, encode_res):
        gt_label = []

        gt_label.append(self.label2id_map[label])
        gt_label.extend([self.ignore_idx] * (len(encode_res["input_ids"]) - 1))
        return gt_label
