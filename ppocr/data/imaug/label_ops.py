...
class IGTRLabelEncode(BaseRecLabelEncode):
    ...
    def __call__(self, data):
        ...
        text = text + [self.dict["<pad>"]] * (self.max_text_len + 2 - len(text))
    ...
    def encode(self, text):
        ...
        if len_ == 1:
            ...
            for _ in range(1, self.k):
                ques_list.append([[self.max_text_len + 2, self.dict["<pad>"], 0]])
            ...
            for slice_id in shuffle_id1:
                if slice_id == 0:
                    ...
                    ques_list.append(
                        qa_text[slice_id:] + [[pos_i + 1, self.dict["</s>"], 0]]
                    )
            ...
            if len_ > 2:
                ...
                ques_list.append(qa_text[1:] + [[pos_i + 1, self.dict["</s>"], 0]])
                prompt_list.append(qa_text[:1])
                ques_list.append(qa_text[:1] + [[pos_i + 1, self.dict["</s>"], 0]])
                prompt_list.append(qa_text[1:])
