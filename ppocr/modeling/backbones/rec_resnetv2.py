...
class StdConv2dSame(nn.Conv2D):
    def __init__(
            ...
            dilation=1, groups=1, bias_attr=False, eps=1e-6, is_export = False):
    ...
    def forward(self, x):
        ...
        if self.export:
            weight = paddle.reshape( F.batch_norm(
                self.weight.reshape([1, self._out_channels,  -1]), running_mean, running_variance,
                momentum=0., epsilon=self.eps, use_global_stats = False),self.weight.shape)
        else:
            weight = paddle.reshape( F.batch_norm(
                self.weight.reshape([1, self._out_channels,  -1]), running_mean, running_variance,
                training = True, momentum=0., epsilon=self.eps,),self.weight.shape)
        x = F.conv2d(x, weight, self.bias, self._stride, self._padding, self._dilation, self._groups)
        return x
...
class StdConv2d(nn.Conv2D):
    ...
    def forward(self, x):
        weight = F.batch_norm(
            ...
            training=True, momentum=0., epsilon=self.eps).reshape_as(self.weight)
...
class MaxPool2dSame(nn.MaxPool2D):
    ...
    """
    def __init__(self, kernel_size: int, stride=None, padding=0, dilation=1, ceil_mode=False, is_export = False):
...
def create_pool2d(pool_type, kernel_size, stride=None, is_export = False, **kwargs):
    ...
    if is_dynamic:
        if pool_type == 'avg':
            return AvgPool2dSame(kernel_size, stride=stride, is_export = is_export, **kwargs)
        elif pool_type == 'max':
            return MaxPool2dSame(kernel_size, stride=stride, is_export = is_export, **kwargs)
        else:
...
def pad_same_export(x, k, s, d = (1, 1), value = 0):
    ih, iw = x.shape[-2:]    
    pad_h, pad_w = get_same_padding_export(ih, k[0], s[0], d[0]), get_same_padding_export(iw, k[1], s[1], d[1])
    pad_h = pad_h.cast(paddle.int32)
    pad_w = pad_w.cast(paddle.int32)
    pad_list= paddle.to_tensor([(pad_w // 2), (pad_w - pad_w // 2).cast(paddle.int32),( pad_h // 2).cast(paddle.int32), (pad_h - pad_h // 2).cast(paddle.int32)])
...
class EvoNormBatch2d(nn.Layer):
    def __init__(self, num_features, apply_act=True, momentum=0.1, eps=1e-5, drop_block=None):
        ...
        self.eps = eps
        self.weight = paddle.create_parameter(paddle.ones(num_features), dtype="float32")
        self.bias = paddle.create_parameter(paddle.zeros(num_features), dtype="float32")
        self.v = paddle.create_parameter(paddle.ones(num_features), dtype="float32") if apply_act else None
...
class GroupNormAct(nn.GroupNorm):
    ...
    def forward(self, x):
        x = group_norm(x, self._num_groups, self._epsilon,   weight =self.weight, bias=self.bias)
        x = self.act(x)
        return x
...
class PreActBottleneck(nn.Layer):
    ...
    def __init__(
            ...
            act_layer=None, conv_layer=None, norm_layer=None, proj_layer=None, drop_path_rate=0., is_export = False):
        super().__init__()
        ...
        self.conv2 = conv_layer(mid_chs, mid_chs, 3, stride=stride, dilation=first_dilation, groups=groups, is_export = is_export)
    ...
    def __init__(
        ...
        if proj_layer is not None:
            self.downsample = proj_layer(
                ...
                conv_layer=conv_layer, norm_layer=norm_layer, is_export = is_export)
        else:
            self.downsample = None
...
class DownsampleConv(nn.Layer):
    def __init__(
            ...
            conv_layer=None, norm_layer=None, is_export = False):
...
class DownsampleAvg(nn.Layer):
    def __init__(
            ...
            preact=True, conv_layer=None, norm_layer=None, is_export = False):
...
class ResNetStage(nn.Layer):
    ...
    def __init__(self, in_chs, out_chs, stride, dilation, depth, bottle_ratio=0.25, groups=1,
                 avg_down=False, block_dpr=None, block_fn=PreActBottleneck, is_export = False,
        ...
        for block_idx in range(depth):
            ...
            self.blocks.add_sublayer(str(block_idx), block_fn(
                prev_chs, out_chs, stride=stride, dilation=dilation, bottle_ratio=bottle_ratio, groups=groups,
                first_dilation=first_dilation, proj_layer=proj_layer, drop_path_rate=drop_path_rate, is_export = is_export,
...
def create_resnetv2_stem(
        ...
        conv_layer=StdConv2d, norm_layer=partial(GroupNormAct, num_groups=32), is_export = False):
    stem = OrderedDict()
    ...
    if is_stem_deep(stem_type):
        ...
        stem['conv1'] = conv_layer(in_chs, stem_chs[0], kernel_size=3, stride=2, is_export = is_export)
        stem['norm1'] = norm_layer(stem_chs[0])
        stem['conv2'] = conv_layer(stem_chs[0], stem_chs[1], kernel_size=3, stride=1, is_export = is_export)
        stem['norm2'] = norm_layer(stem_chs[1])
        stem['conv3'] = conv_layer(stem_chs[1], out_chs, kernel_size=3, stride=1, is_export = is_export)
        if not preact:
            stem['norm3'] = norm_layer(out_chs)
    else:
        # The usual 7x7 stem conv
        stem['conv'] = conv_layer(in_chs, out_chs, kernel_size=7, stride=2, is_export = is_export)
        if not preact:
            stem['norm'] = norm_layer(out_chs)

    if 'fixed' in stem_type:
        # 'fixed' SAME padding approximation that is used in BiT models
        stem['pad'] = paddle.nn.Pad2D(1,
                        mode='constant',
                        value=0.0,
                        data_format='NCHW',
                        name=None)
        ...
        stem['pool'] = create_pool2d('max', kernel_size=3, stride=2, padding='same', is_export = is_export)
    else:
...
class ResNetV2(nn.Layer):
    ...
    def __init__(
            ...
            drop_rate=0., drop_path_rate=0., zero_init_last=False, is_export = False):
        super().__init__()
        ...
        self.stem = create_resnetv2_stem(
            in_chans, stem_chs, stem_type, preact, conv_layer=conv_layer, norm_layer=norm_layer, is_export = is_export)
        ...
        curr_stride = 4
        dilation = 1
        block_dprs = [x.tolist() for x in paddle.linspace(0, drop_path_rate, sum(layers)).split(layers)]
