...
class HWAttention(nn.Layer):
    ...
    def forward(self, x):
        B, N, C = x.shape
        C = C//3
        qkv = x.reshape([B, N, 3, C//self.head_dim, self.head_dim]).transpose([2, 0, 3, 1, 4])
        q, k, v = qkv.unbind(0)
...
class SLAHead(nn.Layer):
    def __init__(self,
        ...
        layer_list = [Block(in_channels, num_heads=2, mlp_ratio=4.0, qkv_bias=True, drop_path=dpr[i]) for i in range(2)]
    ...
    def forward(self, inputs, targets=None):
        ...
        if self.training and targets is not None:
            ...
            structure_ids = paddle.zeros((batch_size, self.max_text_length + 1), dtype=paddle.int64)
