...
export const DEFAULT_DET_MODEL_PARSE_FALLBACKS = Object.freeze({
  resizeLong: 960,
  normalize: {
...
export const DEFAULT_DET_MODEL_CONFIG = Object.freeze({
  ...DEFAULT_DET_MODEL_PARSE_FALLBACKS,
  maxSideLimit: DEFAULT_DET_RUNTIME_LIMITS.maxSideLimit
});

export function parseDetModelConfigText(text) {
  const parsed = parseInferenceConfigText(text);
  const resize = getTransformOp(parsed?.PreProcess?.transform_ops, "DetResizeForTest");
  ...
  return {
    ...
    normalize: {
      mean: normalize?.mean || DEFAULT_DET_MODEL_PARSE_FALLBACKS.normalize.mean,
      std: normalize?.std || DEFAULT_DET_MODEL_PARSE_FALLBACKS.normalize.std,
...
export async function createDetModel({ ort, modelBytes, configText, backend, webgpuState }) {
...
export async function createDetModelSession(ort, modelBytes, backend, webgpuState) {
...
export function preprocessDet(context, sourceMat, params) {
...
export async function runDetModel(context, sourceMat, params) {
...
export function dbPostprocess(context, detOutput, meta, detThresh, boxThresh, unclipRatio) {
...
export function cropByPoly(cv, srcMat, poly) {
