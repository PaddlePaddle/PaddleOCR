...
def rename_and_move_files(logger, src_folder, dst_folder, compare_file_end, compare_string, replace_string):

    for file in os.listdir(src_folder):
        if src_folder == pdfoutputimg_folder_cover and not file.endswith(compare_file_end):
            logger.error('--------------start file rename and move in cover-----------------')

            file_extension = os.path.splitext(file)[1]
            file_name = os.path.splitext(file)[0]
            if 'cover' in file_name:
                for i in range(1, 53):
                    compare_string = f"_page_{i}_cover"
                    if i <= 8:
                        # 執行處理 page1-8 的檔案
            ...
            logger.error('--------------start file rename and move in main-----------------')

            process_file(file, src_folder, dst_folder, compare_string, replace_string)

    logger.error('--------------finish file rename and move-----------------')
...
# 剪切圖片
def crop_img(logger, img_path):
    logger.error('process'+img_path)
    ...
    if ori_img.shape[0] == 0 or ori_img.shape[1] == 0:
        logger.error("Error: Image has zero dimensions.")
        return None
...
def convert_pdf_to_images(logger):
    logger.error('--------------start convert pdf to images-----------------')
            ...
            if filename.endswith('.pdf'):
                ...
                for i, page in enumerate(pages):
                    ...
                    if page.size[0] < 2000:
                        ...
                        page.save(output_path, 'JPEG')
    logger.error('--------------finish convert pdf to images-----------------')

def process_files(logger):
    logger.error('--------------start process files-----------------')

    rename_and_move_files(logger, pdfoutputimg_folder_cover, pdfoutputimg_folder_main, '_page_1_cover.jpg', 'cover', 'main')
    rename_and_move_files(logger, pdfoutputimg_folder_main, pdfoutputimg_folder_cover, '_page_1_main.jpg', 'main', 'cover')

    logger.error('--------------start crop img-----------------')
    for filename in os.listdir(pdfoutputimg_folder_main):
        img_path = os.path.join(pdfoutputimg_folder_main, filename)
        aftercrop_img = crop_img(logger, img_path)
        
        if aftercrop_img is not None:
            cv2.imwrite(img_path, aftercrop_img)
        else:
            logger.error("Error: Failed to crop image: {}".format(img_path))

    logger.error('--------------finish crop img-----------------')

    logger.error('--------------finish process files-----------------')

def perform_ocr(logger):
    logger.error('--------------start process ocr-----------------')
    for filename in os.listdir(pdfoutputimg_folder_main):
        ...
        with open(output_path, mode) as f:
            for righttop in righttop_order[0]:
                for line in result:
                    ...
                    continue
    logger.error('--------------finish process ocr-----------------')
...
def main():
    ...
    create_folders()
    convert_pdf_to_images(logger)
    process_files(logger)
    perform_ocr(logger)
