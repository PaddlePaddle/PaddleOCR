...
* **最先端のドキュメントVLM**: 業界をリードする軽量視覚言語モデル **PaddleOCR-VL-1.6（0.9B）** を搭載。**歪み、スキャン、画面撮影、照明、傾き**という5つの主要な「実環境」課題にわたる複雑なドキュメント解析に優れ、**Markdown**および**JSON**形式の構造化出力に対応しています。
...
* **PaddleOCR-VL-1.6（最新の0.9B VLM）**: ドキュメント解析のための最新フラッグシップモデルが公開されました！
